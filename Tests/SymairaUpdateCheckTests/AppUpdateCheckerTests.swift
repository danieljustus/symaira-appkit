...
final class AppUpdateCheckerTests: XCTestCase {
    ...
    @MainActor
    func testUserDefaultsStoreUsesConfigurableKey() {
        let key = "com.test.myapp.updateSkippedTag"
        let store = UserDefaultsSkippedVersionStore(key: key)
        // Clean up before
        store.setSkippedTag(nil)
        XCTAssertNil(store.skippedTag())

        store.setSkippedTag("v2.0.0")
        XCTAssertEqual(store.skippedTag(), "v2.0.0")

        // Ensure the key is configurable by verifying a different key is not affected
        let otherStore = UserDefaultsSkippedVersionStore(key: "com.test.other.updateSkippedTag")
        XCTAssertNil(otherStore.skippedTag())
